...
public class PNG {
    ...
    public static double[][] extractImage() throws IOException
    {
...
*/
        ...
        for (int i=0;i<width;i++) {
        	for (int j=0; j<height;j++) {
        		p = bui.getRGB(i,j);
        	    c = new Color(p);
        	        X[i+j*height][0] = (double) c.getRed()/255.0;
        	        X[i+j*height][1] = (double) c.getGreen()/255.0;
        	        X[i+j*height][2] = (double) c.getBlue()/255.0;
        	}
        }
        return X;
    } 
